...
public class GradeController {
	...
	public String listByPage(@PathVariable(name="pageNum") int pageNum, Model model,
		...
		long startCount = (pageNum - 1) * GradeService.GRADE_PER_PAGE + 1;
		long endCount = startCount + GradeService.GRADE_PER_PAGE - 1;
	...
	public String newGrade(Model model) {
		...
		if (opLevel.isPresent()) {
			...
			model.addAttribute(SystemConstant.LINK, "grades");
			StaticUtil.setTitleAndStatic(model, SystemConstant.TITLE_CREATE_NEW_GRADE);
	...
	public String editGrade(@PathVariable(name = "id") Long id, RedirectAttributes redirectAttributes, Model model) {
		try {
			...
			if (opGrade.isPresent() && opLevel.isPresent()) {
				...
				title.append(SystemConstant.TITLE_EDIT_GRADE).append(id);
	...
	public String deleteGrade(@PathVariable(name = "id") Long id, RedirectAttributes redirectAttributes) {
		try {
			...
			redirectAttributes.addFlashAttribute(SystemConstant.ATTR_MESSAGE,
					SystemConstant.ATTR_CONTENT_GRADE_DEL_SUCCESS(id));
		} catch (EntityNotFoundException e) {
			redirectAttributes.addFlashAttribute(SystemConstant.ATTR_MESSAGE, e.getMessage());
		}
		return "redirect:/grades";
